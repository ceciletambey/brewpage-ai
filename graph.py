...
Each node reads/writes a shared PipelineState. The graph is compiled once
and reused; app.py streams it node-by-node so the UI can show per-step
progress.
"""
...
# ----------------------------------------------------------------------------
# GRAPH — generation pipeline (strategist -> generator)
# ----------------------------------------------------------------------------
STEP_LABELS = {
...
# ----------------------------------------------------------------------------
# GRAPH — editor loop (single editor node, invoked once per revision)
# ----------------------------------------------------------------------------
_editor_pipeline = None


def build_editor_pipeline():
    graph = StateGraph(PipelineState)
...
def get_editor_pipeline():
    ...
    if _editor_pipeline is None:
        _editor_pipeline = build_editor_pipeline()
